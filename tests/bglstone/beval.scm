;*---------------------------------------------------------------------*/
;*    serrano/prgm/project/bbv/tests/bglstone/beval.scm                */
;*                                                                     */
;*    Author      :  Manuel Serrano                                    */
;*    Creation    :  Wed Feb 10 08:35:54 1993                          */
;*    Last change :  Tue Nov 15 10:56:31 2022 (serrano)                */
;*                                                                     */
...
(define (ewal exp . env)
   (for-each (lambda (x)
		(meaning (compile-define (car x) (cdr x)) '()))
	     env)
...
(define (compile-global-application proc actuals tail?)
   (case (length actuals)
      ((0 1 2 3)
       `(,(SFX+ (length actuals) 27) ,proc ,@actuals))
      (else
       `(32 ,proc ,@actuals))))
...
(define (compile-compiled-application proc actuals tail?)
   (case (length actuals)
      ((0 1 2 3)
       `(,(SFX+ (length actuals) 33) ,proc ,@actuals))
      (else
       `(38 ,proc ,@actuals))))
...
(define (compile-application proc actuals tail?)
   (case (length actuals)
      ((0 1 2 3)
       `(,(SFX+ (length actuals) 39) ,proc ,@actuals))
      (else
       `(44 ,proc ,@actuals))))
...
;*    run ...                                                          */
;*---------------------------------------------------------------------*/
(define (run #!key (num (unknown 60 10)))
