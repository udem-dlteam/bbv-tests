...
(define (get-lemmas s)
  (cond  ((local-assq s lemmas-prop-list) => cdr)
	 (else '())))

(define (put-lemmas s v)
  (cond ((local-assq s lemmas-prop-list) => (lambda (p) (set-cdr! p v)))
...
(define (add-lemma term)
  (cond ((and (pair? term)
              (eq? (car term)
                   'equal)
              (pair? (cadr term)))
         (put-lemmas (car (cadr term))
		     (cons term (get-lemmas (car (cadr term))))))
