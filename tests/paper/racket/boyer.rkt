...
(define (list-deep-copy l)
   (if (mpair? l)
       (mcons (list-deep-copy (Smcar l)) (list-deep-copy (Smcdr l)))
       l))
...
(define (get-lemmas s)
  (cond  ((Smassq s lemmas-prop-list) => (lambda (x) (Smcdr x)))
	 (else '())))

(define (put-lemmas s v)
  (cond ((Smassq s lemmas-prop-list) => (lambda (p) (Sset-mcdr! p v)))
	(else (set! lemmas-prop-list (mcons (mcons s v) lemmas-prop-list)))))
...
(define (add-lemma term)
  (cond ((and (mpair? term)
              (eq? (Smcar term)
                   'equal)
              (mpair? (Smcadr term)))
         (put-lemmas (Smcar (Smcadr term))
		     (mcons term (get-lemmas (Smcar (Smcadr term))))))
        (else (error "ADD-LEMMA did not like term:  " term '()))))

(define (add-lemma-lst lst)
  (cond ((null? lst)
         #t)
        (else (add-lemma (Smcar lst))
              (add-lemma-lst (Smcdr lst)))))

(define (apply-subst alist term)
  (cond ((not (mpair? term))
         (cond ((begin (set! temp-temp (Smassq term alist))
                       temp-temp)
                (Smcdr temp-temp))
               (else term)))
        (else (mcons (Smcar term)
                    (apply-subst-lst alist (Smcdr term))))))

(define (apply-subst-lst alist lst)
  (cond ((null? lst)
         '())
        (else (mcons (apply-subst alist (Smcar lst))
                    (apply-subst-lst alist (Smcdr lst))))))
...
(define (one-way-unify1 term1 term2)
  (cond ((not (mpair? term2))
         (cond ((begin (set! temp-temp (Smassq term2 unify-subst))
                       temp-temp)
                (equal? term1 (Smcdr temp-temp)))
               (else (set! unify-subst (mcons (mcons term2 term1)
                                             unify-subst))
                     #t)))
        ((not (mpair? term1))
         #f)
        ((eq? (Smcar term1)
              (Smcar term2))
         (one-way-unify1-lst (Smcdr term1)
                             (Smcdr term2)))
        (else #f)))
...
(define (one-way-unify1-lst lst1 lst2)
  (cond ((null? lst1)
         #t)
        ((one-way-unify1 (Smcar lst1)
                         (Smcar lst2))
         (one-way-unify1-lst (Smcdr lst1)
                             (Smcdr lst2)))
        (else #f)))
...
(define (rewrite term)
  (cond ((not (mpair? term))
         term)
        (else (rewrite-with-lemmas (mcons (Smcar term)
                                         (rewrite-args (Smcdr term)))
				   (get-lemmas (Smcar term))))))

(define (rewrite-args lst)
  (cond ((null? lst)
         '())
        (else (mcons (rewrite (Smcar lst))
                    (rewrite-args (Smcdr lst))))))

(define (rewrite-with-lemmas term lst)
  (cond ((null? lst)
         term)
        ((one-way-unify term (Smcadr (Smcar lst)))
         (rewrite (apply-subst unify-subst (Smcaddr (Smcar lst)))))
        (else (rewrite-with-lemmas term (Smcdr lst)))))
...
;;; Rewritten as not to use get and put 
   (set! lemmas-prop-list '())
  (add-lemma-lst
   (list-deep-copy
      (mquote ((equal (compile form)
		 ...
		 (if (assignedp x a)
		     (assignment x a)
		     (assignment x b)))
	      (equal (Smcar (gopher x))
		 (if (listp x)
		     (Smcar (flatten x))
		     (zero)))
	      (equal (flatten (Smcdr (gopher x)))
		 (if (listp x)
		     (Smcdr (flatten x))
		     (cons (zero)
			(nil))))
...
(define (tautologyp x true-lst false-lst)
  (cond ((truep x true-lst)
        ...
        ((eq? (Smcar x)
         ...
         (cond ((truep (Smcadr x)
                       true-lst)
                (tautologyp (Smcaddr x)
                            true-lst false-lst))
               ((falsep (Smcadr x)
                        false-lst)
                (tautologyp (Smcadddr x)
                            true-lst false-lst))
               (else (and (tautologyp (Smcaddr x)
                                      (mcons (Smcadr x)
                                            true-lst)
                                      false-lst)
                          (tautologyp (Smcadddr x)
...
(define (size term)
   (if (mpair? term)
       (+ (size (Smcar term)) (size (Smcdr term)))
       1))
        ...
        (apply-subst
         (mquote ((x f (plus (plus a b)
                 ...
                 (w lessp (remainder a b)
                          (member a (length b)))))
         (mquote (implies (and (implies x y)
                              (and (implies y z)
                                   (and (implies z u)
                                        (implies u w))))
                         (implies x w)))))
...
(define (check result)
   (equal? result (mcons #f 136))) ;; in other benchmarks the answer is #t, but some typo causes it to be #f here
