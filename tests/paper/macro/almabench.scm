...
(define (planetpv epoch np pv)
   (let* ;; working storage
	 ...
	 (set! da (SFL+ da
      ...
      (let* ((am (SFL- dl dp))
	     (ae (SFL+ am (SFL* de  (SFLsin am)))))
	 (let loop ( (k 0.) )
	    (let ((dae (SFL/ (SFL+ (SFL- am ae) (SFL* de (SFLsin ae)))
			    (SFL- 1.0 (SFL* de (SFLcos ae))))))
	       (set! ae (SFL+ ae dae))
	 ...
	 ;      ((or (SFL>= k 11)
	 ;           (SFL< (abs dae) 1.0e-12)))
	 ;    (set! dae (SFL/ (SFL- am (SFL+ ae (SFL* de (SFLsin ae))))
	 ;             (SFL- 1.0 (SFL* de (SFLcos ae)))))
	 ;    (set! ae (SFL+ ae dae)))
	 ;;true anomaly.
	 (let* ((ae2 (SFL/ ae 2.0))
		(at  (SFL* 2.0 (GENatan2 (SFL* (SFLsqrt (SFL/ (SFL+ 1.0 de)
					     ...
					     (SFLsin ae2))
					(SFLcos ae2))))
		(r (SFL* da (SFL- 1.0  (SFL* de (SFLcos ae)))))
		(v (SFL* GAUSSK (SFLsqrt (SFL/ (SFL+ 1.0  (SFL/ 1.0 (Svector-ref amas
									 np)))
					  (SFL* da  (SFL* da  da))))))
		(si2   (SFLsin (SFL/ di 2.0)))
		(xq    (SFL* si2  (SFLcos doh)))
		(xp    (SFL* si2 (SFLsin doh)))
		(tl    (SFL+ at  dp))
		(xsw   (SFLsin tl))
		(xcw   (SFLcos tl))
		(xm2   (SFL* 2.0 (SFL- (SFL* xp  xcw) (SFL* xq  xsw))))
		(xf    (SFL/ da (SFLsqrt (SFL- 1.0 (SFL* de de)))))
		(ci2   (SFLcos (SFL/ di 2.0)))
		(xms   (SFL* (SFL+ (SFL* de (SFLsin dp))  xsw)
			    xf))
		(xmc   (SFL* (SFL+ (SFL* de  (SFLcos dp)) xcw)  xf))
...
(define (radecdist state  rdd)
  ...
  ;;Declination
  (Svector-set! rdd 1 (SFL* (SFLasin (SFL/ (Svector-ref (Svector-ref state 0) 2)
