...
function makeMatrix(rows, cols) {
   const mx = new Array(rows).fill(new Array(0));

   for (let i = 0; i < rows; i++) {
      const row = new Array(cols).fill(0.0);
...
const JCENTURY  = 36525.0;
const JMILLENIA = 365250.0;
const TWOPI     = 2.0 * Math.PI;
const A2R       = Math.PI / 648000.0;
...
const TEST_LOOPS = 4;
const TEST_LENGTH = eval("(x, y) => x")(36525, 30);

// sin and cos of j2000 mean obliquity (iau 1976)
const sineps = 0.3977771559319137;
const coseps = 0.9174820620691818;

const amas = [ 6023600.0, 408523.5, 328900.5, 3098710.0, 1047.355, 3498.5, 22869.0, 19314.0 ];
...
function planetpv ( epoch,  np, pv)
{
    // working storage
    let k;
    let t, da, dl, de, dp, di, doh, dmu, arga, argl, am;
    let ae, dae, ae2, at, r, v, si2, xq, xp, tl, xsw;
    let xcw, xm2, xf, ci2, xms, xmc, xpxq2, x, y, z;
    ...
    for (k = 0; k < 8; ++k)
    {
        arga = kp[np][k] * dmu;
        argl = kq[np][k] * dmu;
        da   += (ca[np][k] * cos(arga) + sa[np][k] * sin(arga)) * 0.0000001;
        dl   += (cl[np][k] * cos(argl) + sl[np][k] * sin(argl)) * 0.0000001;
    }

    arga = kp[np][8] * dmu;
    da   += t * (ca[np][8] * cos(arga) + sa[np][8] * sin(arga)) * 0.0000001;

    for (k = 8; k <= 9; ++k)
    {
        argl = kq[np][k] * dmu;
        dl   += t * ( cl[np][k] * cos(argl) + sl[np][k] * sin(argl) ) * 0.0000001;
    }
    ...
    do
    {
        ...
        if ((k >= 10) || (abs(dae) < 1e-12))
            break;
    }
    while ((k < 10) && (abs(dae) >= 1e-12));
...
function run(o)
{
   const K = o?.K || TEST_LENGTH;

   let jd = new Array(2).fill(0.0);
   let pv = makeMatrix(2, 3);
   let position = new Array(3).fill(0.0);
   ...
   return [position, jd, pv];
}

function check(result) {
   const r = result.length === 3
