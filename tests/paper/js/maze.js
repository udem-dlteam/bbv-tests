...
function rand(state) {   
   ...
   let hi = fx32(seed / Q);
   let lo = fx32(seed % Q);
   let test = fx32(fx32(A * lo) - fx32(R * hi));
   let val = test > 0 ? test : fx32(test + M);
...
function writeChar(c) {
   output = CONS(c, output);
}

function printHexMaze(harr, entrance) {
   let nrows = harr.nrows;
   let ncols = harr.ncols;
   let ncols2 = (ncols >> 1) * 2;
   ...
   for (let c = 0; !(c >= ncols2); c += 2) {
      writeChar(c === entrance ? " " : "_");
      writeChar("/");
      writeChar(dotSpace(harr, nrows - 1, c + 1));
      writeChar("\\");
   }
...
const unknown = eval("(x, y) => x * y");

const outputExpected = `   _       _
 _/.\\_/.\\_/ \\_
/ \\  . _/ \\ / \\
\\_ .\\_/   / \\ /
/. _/ \\ /  _/ \\
...
function run(o) {
   ...
   while (n-- > 0) {
      result = run1(nrows, ncols);
   }
   return result;
}

function check(result) {
   if (listToString(result) !== outputExpected) {
      console.log("R=", listToString(result));
      throw "test failed";
   }
}
