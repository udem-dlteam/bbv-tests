...
const Ls = list( list( "s", list( "a"), list( "s", "s")));
const La = list( "a", "a", "a", "a", "a", "a", "a", "a", "a" );

function test() {
   const p = makeParser(Ls, l => l.map( x => list( x, x ) ));
   const x = p( La );
...
function main(argv) {
   const arg = argv.length > 2 ? argv[2] : "{repeat: 1, n: 10000}"
