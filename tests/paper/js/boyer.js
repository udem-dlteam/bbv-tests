...
function size(term) {
   if (term instanceof pair) {
      return size(term.car) + size(term.cdr);
   } else {
      return 1;
   }
}
...
function test() {
   let ans = false;
   let term = false;
   term = applySubst( quote1, quote2 );

   ans = tautp( term );
   return new pair(ans, size(term));
}
...
function check(result) {
   if (result.car !== true || result.cdr !== 136) {
...
function main(argv) {
   const arg = argv.length > 2 ? argv[2] : "{repeat: 1, n: 500}"
