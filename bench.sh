...
if $execute ; then
  ...
  if test -e $bench.scm ; then
    for SC in $SC_range ; do
      for SG in $SG_range ; do
        for V in $V_range ; do
          for I in $I_range ; do
            printf "(%s $SC %s V=%s I=%s)\n" `($TIME $TIMEARGS $bench.$SC.$I$SG$V.exe repeat: $repeat $args) 2>&1 | fgrep real | sed -e 's/.*0m//' -e 's/s$//'` $SG $V $I >> $bench.results
          done
        done
      done
    done
  ...
  if test -e $bench.c.c ; then
    for CC in $CC_range ; do
      for O in $O_range ; do
        printf "(%s $CC -O%s)\n" `($TIME $TIMEARGS $bench.$CC.O$O.exe repeat: $repeat $args) 2>&1 | fgrep real | sed -e 's/.*0m//' -e 's/s$//'` $O >> $bench.results
      done
    done
  fi
